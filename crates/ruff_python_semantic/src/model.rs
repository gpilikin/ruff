...
impl<'a> SemanticModel<'a> {
    ...
    pub fn resolve_attribute_load(&mut self, attribute: &ast::ExprAttribute) -> ReadResult {
        ...
        for (_index, scope_id) in self.scopes.ancestor_ids(self.scope_id).enumerate() {
            let scope = &self.scopes[scope_id];
            for binding_id in scope.get_all(name_expr.unwrap().id.as_str()) {
                if let BindingKind::SubmoduleImport(binding_kind) = &self.bindings[binding_id].kind
                {
                    if binding_kind.qualified_name.to_string() == full_name {
                        ...
                        match self.bindings[binding_id].kind {
                            ...
                            BindingKind::UnboundException(Some(binding_id)) => {
                                ...
                                self.resolved_names
                                    .insert(name_expr.unwrap().into(), binding_id);
